...
public class DAOOperador {
	...
	public ArrayList<Pago> darPagos() throws SQLException, Exception
	{
			...
			String sql = String.format("SELECT R.FINESTADIA, OP.NOMBRE, OP.CEDULA_NIT, NVL(SUM(PRECIO), 0) AS DINERO_RECIBIDO " + 
					"FROM %1$s.RESERVA R " + 
					"RIGHT JOIN %1$s.RESERVASDEALOJAMIENTO RA ON R.IDRESERVA = RA.IDRESERVA " + 
					"RIGHT JOIN %1$s.ALOJAMIENTOSDEOFERTA AO ON RA.IDALOJAMIENTO = AO.IDALOJAMIENTO " + 
					"RIGHT JOIN %1$s.OFERTA O ON AO.IDOFERTA = O.IDOFERTA " + 
					"RIGHT JOIN %1$s.OPERADORES OP ON O.IDOPERADOR = OP.CEDULA_NIT " + 
					"WHERE CANCELADO = 1 AND R.FINESTADIA BETWEEN SYSDATE - 365 AND SYSDATE " + 
					"GROUP BY OP.NOMBRE, OP.CEDULA_NIT, R.FINESTADIA " + 
					"ORDER BY R.FINESTADIA", USUARIO);
			PreparedStatement prepStmt = conn.prepareStatement(sql);
			recursos.add(prepStmt);
			ResultSet rs = prepStmt.executeQuery();
